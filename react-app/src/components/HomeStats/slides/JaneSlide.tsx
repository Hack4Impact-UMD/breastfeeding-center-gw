...
export default function JaneSlide() {
  ...
  const { data: newClients, isPending: clientsPending, error: clientsError } = useNewJaneClientsCount(startDate, endDate);
  const { data: apptCount, isPending: apptsPending, error: apptsError } = useJaneConsultationsCount(startDate, endDate);

  return (
    <HomeCarouselSlide className="relative h-128 max-h-[500px] p-6 px-12 lg:px-18">
      <svg className="absolute inset-0 z-0 h-full w-full" viewBox="0 0 1355 529" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <>
              <div className="flex flex-row gap-4 lg:gap-8 z-10 items-center justify-center">
                <div className="bg-bcgw-yellow-dark rounded-full p-1 size-10 min-w-10 min-h-10 lg:size-20 lg:min-w-20 lg:min-h-20">
                  <img src={janeIcon} alt="Jane icon" className="size-full object-contain" />
                </div>
                <h1 className="font-semibold text-lg lg:text-4xl">JANE STATS FOR {startDate.monthLong.toUpperCase()} </h1>
              </div>
              <div className="flex flex-col lg:flex-row gap-4 w-full max-w-4xl z-10 justify-center items-center">
                <div className="max-w-120 max-h-63 w-full h-full bg-white rounded-lg flex flex-col items-center justify-center p-4 lg:py-6 lg:gap-4">
                  ...
                  <span className="text-black lg:text-xl text-center">New Clients This Month</span>
                </div>
                ...
                <div className="max-w-120 max-h-63 w-full h-full bg-white rounded-lg flex flex-col items-center justify-center p-4 lg:py-6 lg:gap-4">
                  ...
                  <span className="text-black lg:text-xl text-center">Total Consultations This Month</span>
                </div>
              </div>
            </>
          )
