...
export default function HomePage() {
  ...
  const { data: mostAttendedClasses, isPending: classesPending, error: classesError } = useMostAttendedAcuityClasses(startDate, endDate);
  const { data: acuityAttendance, isPending: attendancePending, error: attendanceError } = useAcuityAttendance(startDate, endDate);
  ...
  return <div className="p-4">
    ...
    {classesPending ? (
      ...
      <p>Most Popular Classes: {mostAttendedClasses?.slice(0, 3).map(c => `${c.class} (${c.attendance})`).join(", ")}</p>
    )
    }
