...
export function hasRoles(roles: Role[]) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const user = await admin.auth().getUser(req.token.uid).catch(() => undefined)
    ...
    if (roles.includes(user.customClaims?.role)) {
      ...
      logger.warn(`hasRoles middleware: User has role ${user.customClaims?.role}, but needed roles ${roles} for route!`)
      return res.status(403).send("Unauthorized");
    }
  }
}
