...
router.get("/all", [isAuthenticated], async (_: Request, res: Response) => {
  ...
  const users = (await usersCollection.get()).docs.map(user => user.data() as User);
...
router.delete("/id/:auth_id", [isAuthenticated, hasRoles(["DIRECTOR", "ADMIN"])], async (req: Request, res: Response) => {
  const userId = req.params.auth_id;
  const currentUserRole = (await auth.getUser(req.token!.uid)).customClaims?.role as Role
...
router.put("/me/email", [isAuthenticated], async (req: Request, res: Response) => {
  const {
    oldEmail,
    newEmail
  } = req.body as {
...
router.put("/id/:id/role", [isAuthenticated, hasRoles(["ADMIN", "DIRECTOR"])], async (req: Request, res: Response) => {
  ...
  if (!userToUpdate || roleLevel[userToUpdate.type] >= roleLevel[currentUserRole]) {
